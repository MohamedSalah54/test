...
const ColorBar = ({ onChangeColor ,resetColors, sliderValue, setSliderValue}) => {
  const handleColorChange = (event) => {
    const hue = event.target.value;  // قيمة الـ hue التي يتم اختيارها
    const saturation = 100; // الحفاظ على الساتوريشن بنسبة 100% (اللون مشبع)
    const lightness = hue > 180 ? 20 : 50; // إذا كانت القيمة كبيرة (نحو الأسود) نخفض اللمعان
  
    const color = `hsl(${hue}, ${saturation}%, ${lightness}%)`;  // استخدام HSL للون
    onChangeColor(color);  // إرسال اللون للـ Parent Component
    setSliderValue(hue); // تحديث قيمة الشريط عند التغيير
